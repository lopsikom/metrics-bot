...
export async function AddTask(server_id : string, chat_id : string, name : string, interval : string){
    const response = await addPendingRequest(QueueEvent.PRISMA_ADD_TASK, {server_id, chat_id, name, interval})
    return response.data
}
