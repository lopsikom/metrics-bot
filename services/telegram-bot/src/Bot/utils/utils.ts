...
export async function getUserByTgId(tgId : string) {
    const response = await addPendingRequest(QueueEvent.PRISMA_GET_USER_BY_TGID, tgId);
    return response.data;
}
export async function createNewUser(tgId : string, first_name : string, last_name? : string, username? : string){
    const response = await addPendingRequest(QueueEvent.PRISMA_CREATE_NEW_USER, {telegram_id : tgId, first_name : first_name, second_name : last_name, login : username});
    return response.data;
}
